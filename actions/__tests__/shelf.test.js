...
import azUrl,{backendURL}  from '../../config/serviceAPI';
import { fetchPartsData, addToCart, setView, removeDataPart } from '../shelf';
...
describe('Data fetching of product parts ', () => {
  ...
  test('get parts and price details', () => {
    nock(host)
      ...
      .reply(200, dataResp);
    nock(host)
      .get(`${azUrl('shelfPageDetails')}/${arr}`)
    ...
    return store
      .dispatch(fetchPartsData(axios, newPath, azUrl('getCatData'), azUrl('shelfPageDetails')))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
    });
  let erMessages = [Error: "Request failed with status code 404"] ;
  test('creates PRICE_FAILURE and PARTS_FAILURE when some system error ', () => {
    ...
    const expectedActions = [
      { type: 'PARTS_REQUESTING' },
      { type: 'PARTS_FAILURE',  err: undefined  }
    ];
    const store = mockStore({ shelf: null });
    return store.dispatch(fetchPartsData(axios, newPath, azUrl('getCatData'), azUrl('shelfPageDetails'))).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
});
  ...
  describe('Add To Cart', () => {
    ...
    xtest('Send details to Add To cart method', () => {
      nock(host)
        .post('', postData)
...
describe('removeDataPart', () => {
  test('check the actions for removeDataPart', () => {
    const expectedActions = [{ type: 'EMPTY_DATA' }];
    const store = mockStore({ checkoutData: {} });
    store.dispatch(removeDataPart());
    expect(store.getActions()).toEqual(expectedActions);
  });
});
